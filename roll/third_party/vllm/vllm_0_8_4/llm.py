...
class Llm084(LLM):
    ...
    def add_requests(
        ...
        if isinstance(sampling_params, SamplingParams):
            sampling_params = [sampling_params] * len(request_ids)
        
        for i, (token_ids, request_id, sampling_param)in enumerate(zip(prompt_token_ids, request_ids, sampling_params)):
    ...
    def abort_request_for_migrate(self, request_id: Union[str, Iterable[str]]) -> List[Any]:
        if envs.VLLM_USE_V1:
            request_ids, migrate_request_ids, migrate_request_states = v1_vllm_abort_requests(self.llm_engine.output_processor, request_ids=request_id)
            self.llm_engine.engine_core.abort_requests(request_ids)
            return migrate_request_states
        return llm_engine_abort_request_for_migrate(self.llm_engine, request_id)
    ...
    for state_queue in [self.waiting, self.running, self.swapped]:
        ...
        for aborted_group in aborted_groups:
            ...
            for seq in aborted_group.get_seqs():
                if seq.is_finished():
                    continue
                seq.ori_status = seq.status
                setattr(seq, 'ori_status', seq.status)
        ...
        all_aborted_groups.extend(aborted_groups)
    return all_aborted_groups



def llm_engine_abort_request_for_migrate(self, request_id: Union[str, Iterable[str]]) -> List[Any]:
    seq_groups = list() 
    for scheduler in self.scheduler:
        seq_group_per_schedule = scheduler_abort_seq_group_for_migrate(scheduler,
            request_id, seq_id_to_seq_group=self.seq_id_to_seq_group)
        seq_groups.extend(seq_group_per_schedule)
    return seq_groups


def v1_vllm_abort_requests(
    self,
    request_ids: Iterable[str],
) -> list[str]:
    request_ids_to_abort = list()
    migrate_request_ids = list()
    migrate_request_states = list()
    assert len(self.parent_requests) == 0
    for request_id in request_ids:
        if request_id not in self.request_states:
            request_ids_to_abort.append(request_id) # key step 
            with open('output/finish_reqs.txt', 'a+') as f:
                    f.write(f"miss {request_id} due to abort \n")
            continue
        # req_state = self.request_states.pop(request_id, None)
        req_state = self.request_states.pop(request_id, None)
        assert req_state.request_id == request_id
        request_ids_to_abort.append(request_id)
        migrate_request_states.append(req_state)
        migrate_request_ids.append(request_id)
    return request_ids_to_abort, migrate_request_ids, migrate_request_states

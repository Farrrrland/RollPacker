...
class RLVRConfig(BaseConfig):
    ...
    drop_generation_num: int = field(
        default=0,
        metadata={
            "help": "intro-group reduant sampling number"
        },
    )
    ...
    max_additional_running_prompts: int = field(
        default=1000, metadata={"help": "The additional number of running prompts, beyond batch_size."}
    )
    dynamic_sampling: bool = field(
        default=True,
        metadata={"help": "Whether to enable dynamic sampling."}
    )
    max_num_return_sequences: int = field(
        default=1,
        metadata={"help": "The max number of return sequences for one promptc."}
    )
    max_prompts: int = field(
        default=16, metadata={"help": "The max number of running prompts, regardless of batch size."}
    )
    max_prompts_ratio: float = field(
        default=1.25, metadata={"help": "The max number of running prompts ratio."}
    )
    multi_infer_tp: bool = field(
        default=False,
        metadata={"help": "Whether to enable multiple inference instances with different tp size."}
    )
    debug_max_steps: int = field(
        default=1000000, metadata={"help": "The max step actally invoked when debugging."}
    )
    is_use_pre_pass_filter: bool = field(
        default=False, metadata={"help": "Whether to use pre pass filter when performing dynamic sampling."}
    )
    ...
    actor_infer_tp: Optional[WorkerConfig] = field(
        default_factory=WorkerConfig,
        metadata={"help": "Configuration for the actor's inference role with larger tp."}
    )
    ...
    dry_run_for_profile: bool = field(default=False)
    dynamic_sampling: bool = field(default=False)
    autoscaling: bool = field(default=False)
    infer_scaling_down_progress_ratio: float = field(default=-1)
    record_time_profiler_log_dir: str = field(default=None)
    scaling_down_train_batch_size: int = field(default=0)
    schedule_steps: List[int] = field(
        default_factory=list,
        metadata={"help": "Steps at which to change the max_new_tokens during training."}
    )
    schedule_max_new_tokens: List[int] = field(
        default_factory=list,
        metadata={"help": "Max new tokens at each step in schedule_steps."}
    )
    fake_old_log_probs: bool = field(default=False)
    first_half_ranks: List[int] = field(default_factory=list)
    second_half_ranks: List[int] = field(default_factory=list)
    max_dp_rank: int = field(default=0)
    fixed_async: bool = field(
        default=False,
        metadata={"help": "Whether to use fixed async mode, which is used for performance testing."}
    )
    second_half_func: bool = field(
        default=False,
        metadata={"help": "Whether to use second half function, which is used for performance testing."}
    )
    oneshot_prompt_count: int = field(
        default=1,
        metadata={"help": "Whether to use one shot all examples, which is used for performance testing."}
    )
    autoscaling_reward_method: str = field(
        default=None,
        metadata={"help": "Whether to use one shot all examples, which is used for performance testing."}
    )
    autoscaling_global_reward_method: str = field(
        default=None,
        metadata={"help": "Whether to use one shot all examples, which is used for performance testing."}
    )
    cutoff: int = field(
    ...
    train_only: bool = field(
        default=False,
        metadata={"help": "Whether to use train_only, which is used for performance testing."}
    )
    train_only_sample_prefix: str = field(
        default=None, 
        metadata={"help": "The filename prefix for train_only samples to load."}
    )
    ...
    def __post_init__(self):
        ...
        if (
                ...
                or self.actor_infer.model_args.model_name_or_path # potential BUG here!
            ...
            if self.multi_infer_tp:
                self.actor_infer_tp.model_args.model_name_or_path = self.pretrain
        ...
        if self.multi_infer_tp and self.actor_infer_tp.worker_cls is None:
            self.actor_infer_tp.worker_cls = "roll.pipeline.base_worker.ActorWorker"
        ...
        if self.multi_infer_tp:
            self.actor_infer_tp.training_args.output_dir = self.output_dir
            self.actor_infer_tp.generating_args.num_return_sequences = self.num_return_sequences_in_group
            self.actor_infer_tp.name = "actor_infer_tp"

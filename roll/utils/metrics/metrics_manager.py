...
class MetricsManager:
    ...
    def add_token_metrics(self, batch, prefix: str = "token") -> Dict[str, Any]:
        ...
        total_lengths = sorted(response_length.cpu().tolist())
        p99 = total_lengths[int(len(total_lengths) * 0.99)]
        p95 = total_lengths[int(len(total_lengths) * 0.95)]
        p90 = total_lengths[int(len(total_lengths) * 0.90)]
        p75 = total_lengths[int(len(total_lengths) * 0.75)]
        p50 = total_lengths[int(len(total_lengths) * 0.50)]
        metrics[f"{prefix}/response_length/p99"] = p99
        metrics[f"{prefix}/response_length/p95"] = p95
        metrics[f"{prefix}/response_length/p90"] = p90
        metrics[f"{prefix}/response_length/p75"] = p75
        metrics[f"{prefix}/response_length/p50"] = p50

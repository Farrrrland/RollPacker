...
class SpecDecodeWorkerHelper:

    def reload_model(self):
        if not self.scorer_worker.model_runner.model:
            self.scorer_worker.model_runner.model_config.load_format = "dummy"
            self.scorer_worker.model_runner.load_model()

    def load_weights(self, weights: Iterable[Tuple[str, torch.Tensor]]):
        # before updating the parameters, we need to reinitialize the previously released model
        self.reload_model()
        # import pdb; pdb.set_trace() 
        patch_vllm_moe_model_weight_loader(self.scorer_worker.model_runner.model)
        
        self.scorer_worker.model_runner.model.load_weights(weights=weights)

    def load_states(self):
        self.reload_model()
        self.scorer_worker.model_runner.model.to('cuda')

    def offload_states(self):
        self.scorer_worker.model_runner.model.to('cpu')
        self.scorer_worker.cache_engine = None
        self.scorer_worker.gpu_cache = None
    ...
    def setup_collective_group(self, comm_plan, backend, rank_in_cluster):
        # import pdb; pdb.set_trace() 
        self.scorer_worker.model_update_comm_plan = getattr(self, "model_update_comm_plan", {})
        ...
        self.scorer_worker.model_update_comm_plan[src_pp_rank] = dict(rank=rank,
    ...
    def broadcast_bucket(self, src_pp_rank, meta_infos, bucket_size):
        if src_pp_rank not in self.scorer_worker.model_update_comm_plan:
            return
        comm_plan = self.scorer_worker.model_update_comm_plan[src_pp_rank]
        ...
        SpecDecodeWorkerHelper.update_parameter_in_bucket(self, meta_infos, buffer, [dist.get_rank()])
    ...
    def broadcast_parameter(self, src_pp_rank, dtype, shape, parameter_name):
        ...
        SpecDecodeWorkerHelper.update_parameter(self, parameter_name, weight, [dist.get_rank()])

...
@ray.remote(concurrency_groups={"multi_thread": 256})
class MathSchedRewardWorker(Worker):
    """
    ...
    def _get_all_child_pids(self):
        try:
            process = psutil.Process(self.pid)
            children = process.children(recursive=True)
            return [child.pid for child in children] + [self.pid]
    ...
    def compute_rewards(self, data: DataProto):
        ...
        # output = DataProto.from_dict(
        #     tensors={
        #         "response_level_rewards": (torch.rand(1) >= 0.5).half(),
        #     }
        # )
        return output

...
from roll.distributed.scheduler.generate_scheduler_batcher_reward import DynamicSamplingSchedulerBatchedReward
...
class RLVRPipelineBatchedReward(BasePipeline):
    ...
    def __init__(self, pipeline_config: RLVRConfig, 
        ...
        self.generate_schedulers: Dict[str, DynamicSamplingSchedulerBatchedReward] = {}
        ...
        for i, domain in enumerate(domain_list):
            ...
            generate_scheduler = DynamicSamplingSchedulerBatchedReward.options(
        ...
        if not self.debug:
            if self.val_dataset:
                ...
                self.val_generate_scheduler = DynamicSamplingSchedulerBatchedReward.options(
                    scheduling_strategy=NodeAffinitySchedulingStrategy(
                        node_id=ray.get_runtime_context().get_node_id(),
                        soft=False,
                    )
        ...
        for domain, reward_cluster in self.rewards.items():
            self.reward_worker_iters[domain] = itertools.cycle(reward_cluster.workers)
    ...
    def run(self):
        ...
        while global_step < self.pipeline_config.max_steps:
            if global_step >= self.pipeline_config.debug_max_steps:
                break
            print(f"[Pipeline] Iteration {global_step} start, time={time.time()}")
            ...
            with tps_timer, Timer(name="step_total", logger=None) as step_total_timer:
                ...
                with Timer(name="step_model_update", logger=None) as step_model_update_timer:
                    ...
                    model_update_metrics: Dict = self.model_update(global_step, 0)
                ...
                with actor_infer_timer, actor_infer_response_timer, Timer(
                    ...
                    for domain, scheduler in self.generate_schedulers.items():
                        ...
                        print(f"Domain: {domain}, domain batch size: {self.domain_batch_size[domain]}")
                    for domain, scheduler_ref in scheduler_refs.items():
                        domain_batchs = ray.get(scheduler_ref, timeout=global_timeout)
                        # import pdb; pdb.set_trace()
                        ...
                        for prompt_id, batch_item in domain_batchs.items():
                            reward_worker = next(self.reward_worker_iters[domain])
                            reward_refs[prompt_id] = reward_worker.compute_rewards.remote(batch_item)
                        
                        rewards_remote_end = time.time()
                        
                        for prompt_id, ref in reward_refs.items():
                            try:

                                if domain == "code_sandbox":
                                    rewards: DataProto = ray.get(ref, timeout=30)
                                else:
                                    rewards: DataProto = ray.get(ref)
                            except Exception as e:
                                token_level_rewards = torch.zeros_like(batch_item.batch["responses"], dtype=torch.float16)
                                response_level_rewards = torch.zeros(len(batch_item.non_tensor_batch["id"]), dtype=torch.float16)
                                scores = torch.zeros(len(batch_item.non_tensor_batch["id"]), dtype=torch.float16)
                            ...
                            domain_batchs[prompt_id].union(rewards)
                            completed_buffers[prompt_id] = [domain_batchs[prompt_id][[idx]] for idx in range(domain_batchs[prompt_id].batch.batch_size[0])]

                        rewards_end = time.time()

                        collect_data = [item for sublist in list(completed_buffers.values())[:] for item in sublist]
                        domain_batch = DataProto.concat(collect_data[: self.domain_batch_size[domain] * 8])
                ...
                with Timer(name="cal_old_log_probs_values", logger=None) as cal_old_logpb_timer:
                    ...
                    ref_log_probs = copy.deepcopy(old_log_probs)
                ...
                metrics_mgr.add_metric("time/ref_log_probs_values", cal_old_logpb_timer.last)
                ...
                if int(os.environ.get("REPORT_LENGTH_AND_REWARDS", "0")):
                    iter_time_json = {
                        ...
                        'rollout_time': rollout_end - iter_start_time,
                        'rewards_remote_time': rewards_remote_end - rollout_end,
                        'rewards_time': rewards_end - rewards_remote_end,
                        'train_time': train_end - rewards_end
                    }
                    get_batch_dir = os.path.join(self.pipeline_config.profiler_output_dir, "time_breakdown")
                    os.makedirs(get_batch_dir, exist_ok=True)
                    filename = f"time-breakdown.jsonl"
                    iter_time_json_file = os.path.join(get_batch_dir, filename)
                    with open(iter_time_json_file, "a") as f:
                        f.write(json.dumps(iter_time_json) + "\n")
    ...
    @torch.no_grad()
    def val(self, prefix='val'):
        val_metrics_mgr = MetricsManager()
        batch = DataProto()

        with Timer(name=f"{prefix}/step_generate", logger=None) as step_generate_timer:
            batch.meta_info["is_offload_states"] = False
            ...
            for reward_cluster in self.rewards.values():
                reward_cluster.offload_states()
        val_metrics_mgr.add_metric(f"time/{prefix}/step_generate", step_generate_timer.last)

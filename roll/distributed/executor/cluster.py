...
class Cluster:
    ...
    def _create_workers(self):
        ...
        for rank, pgs in enumerate(placement_groups):
            ...
            env_vars = {
                ...
                "RAY_DEDUP_LOGS": "0",
                "REPORT_LENGTH_AND_REWARDS": os.environ.get("REPORT_LENGTH_AND_REWARDS", "1"),
        ...
        if all(isinstance(arg, list) for arg in args) and all(isinstance(kwarg, list) for kwarg in kwargs.values()):
            if all(len(arg) == length for arg in args) and all(len(kwarg) == length for kwarg in kwargs.values()):
                ...
                for i in range(length):
                    ...
                    # if len(sliced_args) > 0 and 'train_step' in method_name:
                    #     import pdb; pdb.set_trace()
                    #     print("length of sliced_args == {}".format(len(sliced_args[0])), flush=True)
    ...
    def execute_first_half(self, method_name: str, *args, **kwargs):
        return self.execute_first_half_async(method_name, *args, **kwargs)
    
    def execute_first_half_sync(self, method_name: str, *args, **kwargs):
        return ray.get(self.execute_first_half_async(method_name, *args, **kwargs))
    
    def execute_first_half_async(self, method_name: str, *args, **kwargs):
        assert "_ranks" in kwargs, f"_ranks is not in kwargs"
        ranks = kwargs.get("_ranks", list(range(len(self.workers) // 2)))
        if isinstance(ranks[0], list): # dispatch function may duplicate _ranks
            ranks = ranks[0]
        
        length = len(ranks)
        first_shift_length = ranks[0]
        ...
        if all(isinstance(arg, list) for arg in args) and all(isinstance(kwarg, list) for kwarg in kwargs.values()):
            if all(len(arg) == length for arg in args) and all(len(kwarg) == length for kwarg in kwargs.values()):
                ...
                for i in range(length):
                    sliced_args = tuple(arg[i] for arg in args)
                    sliced_kwargs = {k: v[i] for k, v in kwargs.items()}
                    remote_call = getattr(self.workers[ranks[i]], method_name)
                    result.append(remote_call.remote(*sliced_args, **sliced_kwargs))
                return result

        # return [getattr(worker, method_name).remote(*args, **kwargs) for worker in self.workers[:length]]
        return [getattr(self.workers[rank], method_name).remote(*args, **kwargs) for rank in ranks]

    def execute_second_half(self, method_name: str, *args, **kwargs):
        return self.execute_second_half_async(method_name, *args, **kwargs)
    
    def execute_second_half_sync(self, method_name: str, *args, **kwargs):
        return ray.get(self.execute_second_half_async(method_name, *args, **kwargs))

    def execute_second_half_async(self, method_name: str, *args, **kwargs):
        assert "_ranks" in kwargs, f"_ranks is not in kwargs"
        ranks = kwargs.get("_ranks", list(range(len(self.workers) // 2, len(self.workers))))
        if isinstance(ranks[0], list): # dispatch function may duplicate _ranks
            ranks = ranks[0]
        ...
        # return [getattr(worker, method_name).remote(*args, **kwargs) for worker in self.workers[length:]]
        print("pass ranks == {} in execute_second_half_async".format(ranks), flush=True)
        return [getattr(self.workers[rank], method_name).remote(*args, **kwargs) for rank in ranks]

    def execute_ranks(self, method_name: str, *args, **kwargs):
        return self.execute_ranks_async(method_name, *args, **kwargs)

    def execute_ranks_async(self, method_name: str, *args, **kwargs):
        ranks = kwargs.get("_ranks", None)
        assert ranks is not None, "execute_ranks_async requires _ranks param to be explicitly provided."

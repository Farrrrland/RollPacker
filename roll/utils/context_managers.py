...
def state_offload_manger(strategy, metrics: Dict, metric_infix: str, is_offload_states=True, load_kwargs={}):
    ...
    with Timer(name=f"{metric_infix}_total", logger=None) as timer, local_profiler():
        with Timer(name=f"{metric_infix}_onload", logger=None) as onload_timer, profiling.profile("load_states"):
        ...
        with Timer(name=f"{metric_infix}_execute", logger=None) as execute_timer, profiling.profile("execute"):
            yield

        with Timer(name=f"{metric_infix}_offload", logger=None) as offload_timer, profiling.profile("offload_states"):
            for device_id in range(torch.cuda.device_count()):

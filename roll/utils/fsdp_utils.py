...
@torch.no_grad()
def offload_fsdp_model_gradients_to_cpu(model: FSDP, empty_cache: bool = True):
    assert isinstance(model, FSDP)
    # Lazy init FSDP model
    _lazy_init(model, model)
    assert model._is_root, "Only support root model for gradient offloading"

    for handle in model._all_handles:
        flat_param = handle.flat_param
        
        if flat_param._grad is not None and flat_param._grad.device.type == "cuda" and flat_param._local_shard.device.type == 'cpu':
            if flat_param._grad.size(0) != flat_param.data.size(0): continue
            
            grad_on_cpu = flat_param._grad.to(device="cpu", non_blocking=True)
            flat_param._grad = grad_on_cpu
...
@torch.no_grad()
def load_fsdp_model_gradients_to_gpu(model: FSDP):
    assert isinstance(model, FSDP)
    # Lazy init FSDP model
    _lazy_init(model, model)
    assert model._is_root, "Only support root model for gradient loading to GPU"
    ...
    for handle in model._all_handles:
        ...
        if flat_param.grad is not None and flat_param.grad.device != target_device:
            # Move gradient back to GPU
            grad_on_gpu = flat_param._grad.to(device=target_device, non_blocking=True)
            # print(f"load fsdp model gradients move from device {flat_param.grad.device} -> {target_device}", flush=True)
            # Replace CPU gradient with GPU one
            flat_param._grad = grad_on_gpu

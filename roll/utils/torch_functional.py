...
def allgather_dict_tensors(tensors: Union[Dict[str, torch.Tensor], TensorDict], size, group, dim=0):
    ...
    for key in sorted_keys:
        ...
        if False: 
            ...
            all_tensor_batch = [None for _ in range(size)]
            torch.distributed.all_gather_object(all_tensor_batch, val, group=group)
            output[key] = torch.cat(all_tensor_batch, dim=dim)
            batch_size = output[key].size(0)
            
    if is_tensor_dict:
        output = TensorDict(source=output, batch_size=batch_size)

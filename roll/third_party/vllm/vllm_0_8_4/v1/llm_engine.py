...
class LLMEngine084(LLMEngine):
    ...
    def step_nowait(self) -> list[RequestOutput]:
        ...
        if False:
            waiting_num = outputs.scheduler_stats.num_waiting_reqs
            previous_gen_tokens = self.iteration_stats.num_generation_tokens
        ...
        if False:
            # NOTE: for profiling, print max gen tokens w/o preemption in vllm engine
            if self.previous_waiting_num == 0 and waiting_num > 0:

                print(f"[VLLM DEBUG] [outputs] {len(processed_outputs.request_outputs)} [aborts] {processed_outputs.reqs_to_abort}")

                print(f"[VLLM MAX TOKENS BEFORE PREEMPTION] num_generation_tokens: {previous_gen_tokens}",
                    f"num running requests: {outputs.scheduler_stats.num_running_reqs}",
                    f"num waiting requests: {outputs.scheduler_stats.num_waiting_reqs}")
            self.previous_waiting_num = waiting_num

        return processed_outputs.request_outputs
    
    def reset_iteration_stats(self):
        # NOTE: reset vllm iteration stats to record per iteration metrics
        # print(f"[VLLM ITERATION INFO] total tokens: {self.iteration_stats.num_generation_tokens}, preemptions: {self.iteration_stats.num_preempted_reqs}")

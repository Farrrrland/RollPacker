...
def _communicate_shapes(tensor_send_next, tensor_send_prev, recv_prev, recv_next, config):
    ...
    if config.use_ring_exchange_p2p:
        print("run torch.distributed.ring_exchange", flush=True)
        ...
        ops = []
        print(f"send_prev_shape_tensor is {send_prev_shape_tensor}, recv_prev_shape_tensor {recv_prev_shape_tensor}", flush=True)
        if send_prev_shape_tensor is not None:
            print(f"get_pipeline_model_parallel_prev_rank is {get_pipeline_model_parallel_prev_rank()}")
            send_prev_op = torch.distributed.P2POp(
                torch.distributed.isend,
                send_prev_shape_tensor,
                get_pipeline_model_parallel_prev_rank(),
            )
            print("complete recv op 1", flush=True)
        ...
        if recv_prev_shape_tensor is not None:
            ...
            print("complete recv op 2", flush=True)
            ops.append(recv_prev_op)
        if send_next_shape_tensor is not None:
            print(f"get_pipeline_model_parallel_next_rank is {get_pipeline_model_parallel_next_rank()}")
            send_next_op = torch.distributed.P2POp(
                torch.distributed.isend,
                send_next_shape_tensor,
                get_pipeline_model_parallel_next_rank(),
            )
            print("complete send op 3", flush=True)
        ...
        if recv_next_shape_tensor is not None:
            ...
            print("complete send op 4", flush=True)

...
# from roll.distributed.strategy import p2p_communication
...
def get_shapes_string(variable, name="variable", indent=0):
    """
    返回一个字符串，描述变量中各个 iterable 的 shape。
    支持 dict, list, tuple 的嵌套结构。
    """
    lines = []
    spaces = "  " * indent

    if isinstance(variable, dict):
        lines.append(f"{spaces}{name}: dict")
        for k, v in variable.items():
            lines.extend(get_shapes_string(v, f"{name}['{k}']", indent + 1).splitlines())
    elif isinstance(variable, (list, tuple)):
        type_name = 'list' if isinstance(variable, list) else 'tuple'
        lines.append(f"{spaces}{name}: {type_name}")
        for i, item in enumerate(variable):
            lines.extend(get_shapes_string(item, f"{name}[{i}]", indent + 1).splitlines())
    elif hasattr(variable, 'shape'):
        shape = getattr(variable, 'shape')
        lines.append(f"{spaces}{name}.shape: {shape}")
    else:
        type_name = type(variable).__name__
        lines.append(f"{spaces}{name}: {type_name} (no shape)")
...
def forward_step(
    ...
    # print(f"[Before Forward Schedule] input shape [{get_shapes_string(input_tensor)}], Rank is {dist.get_rank()} ", flush=True)
    ...
    # print(f"[Forward Schedule] output_tensor.shape {output_tensor.shape}, last stage {parallel_state.is_pipeline_last_stage()}, current rank {dist.get_rank()}")


    # if dist.get_rank() == 0: 
    #     import pdb; pdb.set_trace()
    #     print(get_shapes_string(input_tensor))
    #     # input_tensor
    # dist.barrier()
        ...
        if not collect_non_loss_data:
            ...
            if len(outputs) == 3:
                ...
                if not config.calculate_per_token_loss:
                    ...
                    if config.fixed_gradient_accumulation_steps_in_no_sync > 0: 
                        output_tensor /= config.fixed_gradient_accumulation_steps_in_no_sync
                    else:
    ...
    if hasattr(config, 'num_moe_experts') and config.num_moe_experts is not None:
        ...
        if config.calculate_per_token_loss:
            ...
            if config.fixed_gradient_accumulation_steps_in_no_sync > 0: 
                MoEAuxLossAutoScaler.set_loss_scale(loss_scale / config.fixed_gradient_accumulation_steps_in_no_sync)
            else:
    ...
    with no_sync_func():
        for i in range(num_microbatches - 1):
            ...
            if not forward_only:
                ...
                #         print(f"curiter {i}, in mcore_sched {dist.get_rank()}, {bucket.grad_data.norm()}", flush=True)
            # if sub_group is not None:
            #     dist.barrier(sub_group)
    # import pdb; pdb.set_trace()

    # Run computation for last microbatch out of context handler (want to
    # synchronize gradients).

    # with no_sync_func() if getattr(model, '_enable_no_sync', False) else contextlib.nullcontext():
    if True: 
        output_tensor, num_tokens = forward_step(
            forward_step_func,
            data_iterator,
        ...
        total_num_tokens += num_tokens
        # print("starting last backward_step {}".format(config.finalize_model_grads_func), flush=True)
        if not forward_only:
            backward_step(input_tensor, output_tensor, output_tensor_grad, model_type, config)
        # print("finish last backward_step {}".format(config.finalize_model_grads_func), flush=True)

    # print(f"config.finalize_model_grads_func is {config.finalize_model_grads_func}", flush=True)
    # import pdb; pdb.set_trace()
    if not getattr(model, '_enable_no_sync', False):
        if config.finalize_model_grads_func is not None and not forward_only:
            ...
            # print("complete finalize", flush=True)
    elif getattr(model, '_enable_no_sync', False): 
        if config.finalize_model_grads_func is not None:
            if False: 
                print("start finalize with no_sync", flush=True)
                tmp_finish_grad_sync = model.finish_grad_sync
                # import pdb; pdb.set_trace() 
                model.finish_grad_sync = lambda : None  # Disable grad sync in the model.
                config.finalize_model_grads_func(
                        [model], total_num_tokens if config.calculate_per_token_loss else None
                    )
                model.finish_grad_sync = tmp_finish_grad_sync
                print("compl finalize with no_sync", flush=True)
            else: 
                config.finalize_model_grads_func(
                    [model], total_num_tokens if config.calculate_per_token_loss else None
                )
...
def forward_backward_pipelining_with_interleaving(
    *,
    ...
    elif not forward_only and (getattr(model, '_enable_no_sync', False)): 
        # If no finalize_model_grads_func, we need to do the weight gradient GEMM's here.
        # This is needed for the case when we are not using M-Core.
        finish_embedding_wgrad_compute(config, embedding_module)
...
def forward_backward_pipelining_without_interleaving(
    *,
    ...
    if config.finalize_model_grads_func is not None and not forward_only:
        embedding_module = clear_embedding_activation_buffer(config, model)
    elif not forward_only and (getattr(model, '_enable_no_sync', False)): # FIXME: think later
        embedding_module = clear_embedding_activation_buffer(config, model)

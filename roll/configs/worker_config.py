...
class StrategyArguments:
    strategy_name: Literal[
        "deepspeed_train", "hf_infer", "deepspeed_infer", "vllm", "sglang", "megatron_infer", "megatron_train", "fsdp_infer", "fsdp_train"
    ] = field(
        default="deepspeed_train",
        metadata={
            "help": "The name of the strategy. Options: 'deepspeed_train', 'hf_infer', 'deepspeed_infer', 'vllm', 'sglang', "
            "'megatron_infer', 'megatron_train', 'fsdp_infer', 'fsdp_train'."
        },
    )
...
class WorkerConfig:
    ...
    use_remove_padding: bool = field(
        ...
        metadata={
            "help": (
                "Whether to enable use_remove_pading strategy."
            )
        },
    )

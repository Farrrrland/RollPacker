...
source_data_file = 'math_deepmath_deal.jsonl'
target_data_file = 'llm_judge_math_rlvr_new.jsonl'
source_half_file = 'math_deepmath_deal_new.jsonl'

# 按 difficulty 分组存储所有数据
difficulty_to_data = defaultdict(list)

with open(source_data_file, 'r', encoding='utf-8') as f_in:
    for line in f_in:
        math_data = json.loads(line)
        difficulty = math_data['difficulty']
        difficulty_to_data[difficulty].append(math_data)

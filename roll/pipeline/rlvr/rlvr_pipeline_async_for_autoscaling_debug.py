        ...
        for global_step in range(self.pipeline_config.max_steps):
            ...
            with tps_timer, Timer(name="step_total", logger=None) as step_total_timer:
                    ...
                    with actor_infer_timer, actor_infer_response_timer, Timer(
                        ...
                        if self.pipeline_config.autoscaling:
                            ...
                            if False: 
                                ...
                                if prefetch_domain_batches.batch is not None and len(prefetch_domain_batches.meta_info) > 0:
                ...
                if False:
                    ...
                    print("prefetch_domain_batches.batch size {prefetch_domain_batches.batch}", flush=True)
                    actor_train_metrics_refs = self.actor_train.train_step_full(prefetch_domain_batches, blocking=False)


                if False and self.pipeline_config.record_time_profiler_log_dir is not None:
                    profiler_log_dir = self.pipeline_config.record_time_profiler_log_dir
                ...
                if False and self.pipeline_config.second_half_func and self.pipeline_config.autoscaling and self.pipeline_config.scaling_down_train_batch_size > 0 and self.pipeline_config.fixed_async: 
                    scaling_down_train_batch_size = self.pipeline_config.scaling_down_train_batch_size
                    async_training_batch_size = scaling_down_train_batch_size * self.pipeline_config.num_return_sequences_in_group
                    _ranks = copy.deepcopy(self.pipeline_config.second_half_ranks)
                    ...
                    generate_output = generate_output[async_training_batch_size:]
                ...
                for domain, domain_batch in batch_grouped.items():
                    ...
                    with Timer(name="compute_advantage", logger=None) as compute_advantage_timer:
                        ...
                        print("= " * 20)
                        print("in main pipeline", flush=True)
                        print("kl_ctrl is {}, running {}".format(
                            self.kl_ctrl, self.running
                        ), flush=True)
                        print("gamma is {}, lambd is {}, adv_estimator is {}, withen_advantages is {}, whiten_rewards is {}".format(self.pipeline_config.gamma, self.pipeline_config.lambd, 
                                                                                                                                    self.pipeline_config.adv_estimator, self.pipeline_config.whiten_advantages, 
                                                                                                                                    self.pipeline_config.whiten_rewards), flush=True)
                ...
                if self.pipeline_config.generate_opt_level == 2 and len(domain_batches_in_prefetch[MULTI_SCHED_NAME].meta_info) > 0 and (self.pipeline_config.autoscaling_global_reward_method not in ['local']): 
                ...
                if (not self.pipeline_config.second_half_func) and self.pipeline_config.autoscaling and self.pipeline_config.scaling_down_train_batch_size > 0 and self.pipeline_config.fixed_async: 
                    scaling_down_train_batch_size = np.random.randint(
                        self.pipeline_config.scaling_down_train_batch_size, 
                        self.pipeline_config.scaling_down_train_batch_size * 2
                    )
                    _ranks = copy.deepcopy(self.pipeline_config.second_half_ranks)
                    if False: 
                        async_training_batch_size = scaling_down_train_batch_size * self.pipeline_config.num_return_sequences_in_group
                        prefetch_domain_batches = batch[:async_training_batch_size]
                        self.actor_train.train_step_second_half(prefetch_domain_batches, _ranks=_ranks, blocking=True)
                    else:
                        for index in range(self.pipeline_config.num_return_sequences_in_group): 
                            prefetch_domain_batches =  batch[index*self.pipeline_config.num_return_sequences_in_group:(index+1)*self.pipeline_config.num_return_sequences_in_group]
                            self.actor_train.train_step_second_half(prefetch_domain_batches, _ranks=_ranks, blocking=True)
                    
                    batch = batch[scaling_down_train_batch_size*self.pipeline_config.num_return_sequences_in_group:]


                if self.pipeline_config.second_half_func and self.pipeline_config.autoscaling and self.pipeline_config.scaling_down_train_batch_size > 0 and self.pipeline_config.fixed_async: 
                    scaling_down_train_batch_size = self.pipeline_config.scaling_down_train_batch_size
                    async_training_batch_size = scaling_down_train_batch_size * self.pipeline_config.num_return_sequences_in_group
                    _ranks = copy.deepcopy(self.pipeline_config.second_half_ranks)
                    if False:
                        torch.save({'batch':batch.batch}, 'train_async_batch.pt')
                        torch.save({'meta_info':batch.meta_info}, 'train_async_meta.pt')
                    # torch.load('train_async_batch.pt', weights_only=False)
                    # torch.load('train_async_meta.pt', weights_only=False)
                    # import pdb; pdb.set_trace()
                    self.train_second_half_with_func(meta_data=DataProto(meta_info={"global_step": global_step, "is_offload_states": False}), \
                                                        prefetch_actor=self.generate_schedulers[MULTI_SCHED_NAME], 
                                                        metrics_mgr=metrics_mgr,
                                                        _ranks=_ranks,
                                                        record_time_list=record_time_list)
                    batch = batch[scaling_down_train_batch_size*self.pipeline_config.num_return_sequences_in_group:]

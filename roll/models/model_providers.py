...
def default_reward_model_provider(
    ...
    if (
        ...
        if model_args.model_type in ["auto_sequence_classification"]:
            ...
            # print(f"CHECKCHEKCCHECK: {os.environ.get('LAYER_BY_LAYER', '0')}")
            # import pdb; pdb.set_trace()
            
            if os.environ.get("LAYER_BY_LAYER", "0") == "1":
                qwen_model = model.pretrained_model.model
                qwen_model.optimized_embed_tokens = LayerWise(qwen_model.embed_tokens)
                qwen_model.optimized_rotary_emb = LayerWise(qwen_model.rotary_emb)
                qwen_model.optimized_norm = LayerWise(qwen_model.norm)

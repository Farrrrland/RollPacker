...
class RLVRPipelineFixedAsync(BasePipeline):
    ...
    def run(self):
        ...
        for global_step in range(self.pipeline_config.max_steps):
            ...
            with tps_timer, Timer(name="step_total", logger=None) as step_total_timer:
                        ...
                        if self.pipeline_config.autoscaling:
                            ...
                            if False: 
                                ...
                                if prefetch_domain_batches.batch is not None and len(prefetch_domain_batches.batch) > 0:
                                    self.train_second_half(prefetch_domain_batches=prefetch_domain_batches, metrics_mgr=metrics_mgr, record_time_list=record_time_list)
                ...
                if True: 
                    total_batch_size = len(generate_output)
                    prefetch_domain_batches = generate_output[:total_batch_size // 4]
                    prefetch_cnt = len(prefetch_domain_batches.batch) if prefetch_domain_batches.batch is not None else 0
                    metrics_mgr.add_metric("async_time/async_prefetch_count", prefetch_cnt)
                    ...
                    generate_output = generate_output[total_batch_size // 4:]
                ...
                if False: 
                    ...
                    prefetch_domain_batches = batch[:total_batch_size // 4]
                    prefetch_cnt = len(prefetch_domain_batches.batch) if prefetch_domain_batches.batch is not None else 0
                    metrics_mgr.add_metric("async_time/async_prefetch_count", prefetch_cnt)
                    ...
                    batch = batch[total_batch_size // 4:]
                ...
                with Timer(name="step_train", logger=None) as step_train_timer:
                    ...
                    with actor_train_timer:
                        ...
                        if self.pipeline_config.critic_warmup <= global_step:
                            ...
                            if False: 
                                # if self.pipeline_config.autoscaling: 
                                print("length of batch is ", len(batch.batch), flush=True)
                                import pdb; pdb.set_trace()
                                ray.get(self.actor_train.train_step(batch))
                                import pdb; pdb.set_trace()
                                print("start train_step_full", flush=True)
                                ray.get(self.actor_train.train_step_full(batch))
                                print("complete train_step_full", flush=True)
                                import pdb; pdb.set_trace()
                                print("start train_step", flush=True)
                                ray.get(self.actor_train.train_step(batch))
                                print("complete train_step", flush=True)
                                import pdb; pdb.set_trace()
    ...
    def train_second_half(self, prefetch_domain_batches: List[DataProto], metrics_mgr: MetricsManager, _ranks: List[int], **kwargs):
        ...
        with Timer(name=f'{prefix}_step_total', logger=None) as async_step_timer: 
            ...
            with Timer(name=f'{prefix}_step_train_second_half', logger=None) as actor_train_second_half_timer:
                self.actor_train.train_step_second_half(prefetch_domain_batches, _ranks, blocking=True)
